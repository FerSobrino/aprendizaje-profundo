...
- La pregunta ya no es "¿qué tan grande el modelo?" sino **cómo repartir el presupuesto de cómputo en tres lugares**:
    1. **Preentrenamiento**: los retornos de solo escalar datos+parámetros se aplanaron
    2. **Post-entrenamiento con RL**: SFT, RLHF/DPO y RL con recompensas verificables
    3. **Cómputo en inferencia**: dejar al modelo "pensar" más tokens en los problemas difíciles
- Chinchilla (Hoffmann et al., 2022) resolvió el punto 1; los puntos 2 y 3 son la frontera actual y aún no tienen una ley limpia
- Implicación práctica: el mismo modelo puede ser rápido-y-barato o lento-y-caro según cuánto "piensa" — por eso los proveedores venden *modos* y no solo modelos
...
## ¿Alternativas a la atención?

- El costo cuadrático de la atención en la longitud de secuencia motivó alternativas: los **modelos de espacio de estados** (Mamba y sucesores), con costo lineal
- Veredicto a 2026: **no desplazaron a los Transformers** — la atención sigue ganando en tareas de recuerdo preciso; los SSMs sobreviven en arquitecturas híbridas para contextos muy largos
- Lección de criterio: no toda alternativa prometedora reemplaza al estándar; la carga de la prueba está en el reto
...
- Para política pública esto une los dos mundos del curso: imagen satelital + texto (encuestas, registros) ya no son dos pipelines separados, son una sola entrada  
...
- Dirigir, supervisar y auditar agentes tiene su propio deck en este curso
