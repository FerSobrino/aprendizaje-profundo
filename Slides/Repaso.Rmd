...
- ¿Por qué no hacer una búsqueda exhaustiva probando todas las combinaciones de parámetros?
    ...
    - muy costoso si tenemos millones de parámetros (CCNs/RNNs/GANs/LLMs)


## Regresión lineal 
- Recordemos que podemos motivar la regresión lineal con función de pérdida cuadrática asumiendo que las observaciones vienen de observaciones medidas con ruido $N \sim (0,\sigma^2)$
- La verosimilitud de una y en particular la podemos escribir como: 
$$P(x|y) = \frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{1}{2\sigma^2}(y - w^Tx -b)^2}$$
...
- Minimizar el error cuadrático medio medio es equivalente a la estimación de máxima verosimilitud de un modelo lineal bajo el supuesto de ruido gaussiano aditivo
...
## Generalización
- Recordemos que el modelo con el que terminamos depende explicitamente de la selección de los conjuntos de entrenamiento 
- Por lo que por lo general nuestro error de entrenamiento va a estar sesgado y no representa el $R$
...
- Esto es cierto en parte porque tenemos muchos datos paa entrenar estos modelos
...
- La solución dividir nuestros datos en 3: prueba, validación y entrenamiento - El de validación para poder entrenar los hiperparámetros 
