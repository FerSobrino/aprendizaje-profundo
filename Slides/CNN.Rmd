...
- Esto no aprovecha el hecho que las imágenes tienen características particulares distitas a otros tipos de datos
    - pixeles cercanos estas relacionados estadisticamente 
...
## Padding 
- El problema con las capas convoluciones es que vamos a perder los pixeles de los border 
...
- Si definimos $p_h = k_h -1 $ y $p_w = k_w - 1$, entonces $\lfloor{(n_h +  s_h - 1)/s_h}\rfloor x \lfloor{(n_w + s_w - 1)/s_w}\rfloor$
...
- CNN de 8 capas: 5 convoluciones, dos capas completamente conectas y una capa de salida completamente conectada, utiliza ReLU como función de activación 
...
- Se hizo fine tunning para que pudiera detectar objetos 
